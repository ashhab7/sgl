...
class Scheduler:
    ...
    def event_loop(self):
        while True:
            ...
            # print(f'at the very end {self.indicator}')
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        #print(f'Check here can run list = {can_run_list}')
        ...
        #print(f'waiting queue {self.waiting_queue}')
        ...
        if self.tp_rank == 0:
            ...
            if num_mixed_running > 0:
                ...
                logger.info(
					#f"Ami upre na niche asi"
                    f"it counter : {self.it_counter}   "
                    f"Prefill batch. "
                    f"#new-seq: {len(can_run_list)}, "
                    f"#new-token: {adder.log_input_tokens}, "
                    f"#cached-token: {adder.log_hit_tokens}, "
        ...
        new_batch.prepare_for_extend(self.model_config.vocab_size,wait_tensor = self.recv_reqs_list,it_counter = self.it_counter)
        # extra added for testing
        # new_batch.prepare_for_extend(self.model_config.vocab_size,wait_tensor = self.wait_tensor,it_counter = self.it_counter)

        # Mixed-style chunked prefill
        decoding_reqs = []
        if self.is_mixed_chunk and self.running_batch is not None:
            self.running_batch.prepare_for_decode(wait_tensor=self.recv_reqs_list, it_counter = self.it_counter)
    ...
    def get_new_batch_decode(self) -> Optional[ScheduleBatch]:
        ...
        if not batch.check_decode_mem(wait_tensor = self.recv_reqs_list, it_counter = self.it_counter):
            old_ratio = self.new_token_ratio

            retracted_reqs, new_token_ratio = batch.retract_decode(wait_tensor = self.recv_reqs_list, it_counter = self.it_counter)
        ...
        batch.prepare_for_decode(wait_tensor = self.recv_reqs_list, it_counter = self.it_counter)
        return batch
...
def run_scheduler_process(
    ...
    try:
        scheduler = Scheduler(server_args, port_args, gpu_id, tp_rank,[],1)
