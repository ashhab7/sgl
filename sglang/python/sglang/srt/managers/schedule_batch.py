...
class ScheduleBatch:
    ...
    def alloc_token_slots(self, num_tokens: int,wait_tensor = None, it_counter = None):
        out_cache_loc = self.token_to_kv_pool.alloc(num_tokens)

        if out_cache_loc is None:
            if self.tree_cache is not None:
                self.tree_cache.evict(num_tokens, self.token_to_kv_pool.free,wait_tensor=wait_tensor, it_counter = it_counter)
                out_cache_loc = self.token_to_kv_pool.alloc(num_tokens)
    ...
    def prepare_for_extend(self, vocab_size: int, wait_tensor = None, it_counter = None):
        ...
        out_cache_loc = self.alloc_token_slots(extend_num_tokens,wait_tensor = wait_tensor, it_counter = it_counter)
    ...
    def check_decode_mem(self,wait_tensor = None, it_counter = None):
        bs = len(self.reqs)
        if self.token_to_kv_pool.available_size() >= bs:
            return True

        self.tree_cache.evict(bs, self.token_to_kv_pool.free,wait_tensor = wait_tensor, it_counter = it_counter)
    ...
    def retract_decode(self,wait_tensor = None, it_counter = None):
        ...
        while (
            ...
            if isinstance(self.tree_cache, ChunkCache):
                ...
                self.tree_cache.evict(residual_size, self.token_to_kv_pool.free,wait_tensor, it_counter = it_counter)
    ...
    def prepare_for_decode(self, input_ids=None,wait_tensor = None, it_counter = None):
        ...
        # Alloc mem
        bs = len(self.reqs)
        self.out_cache_loc = self.alloc_token_slots(bs,wait_tensor = wait_tensor,it_counter = it_counter)
